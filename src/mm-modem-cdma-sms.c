...
static void impl_cdma_modem_sms_delete (MMModemCdmaSms *modem,
                                       guint idx,
                                       DBusGMethodInvocation *context);

static void impl_cdma_modem_sms_get (MMModemCdmaSms *modem,
                                    guint idx,
                                    DBusGMethodInvocation *context);

static void impl_cdma_modem_sms_get_format (MMModemCdmaSms *modem,
                                           DBusGMethodInvocation *context);

static void impl_cdma_modem_sms_set_format (MMModemCdmaSms *modem,
                                           guint format,
                                           DBusGMethodInvocation *context);

static void impl_cdma_modem_sms_get_smsc (MMModemCdmaSms *modem,
                                         DBusGMethodInvocation *context);

static void impl_cdma_modem_sms_set_smsc (MMModemCdmaSms *modem,
                                         const char *smsc,
                                         DBusGMethodInvocation *context);

static void impl_cdma_modem_sms_list (MMModemCdmaSms *modem,
                                     DBusGMethodInvocation *context);

static void impl_cdma_modem_sms_save (MMModemCdmaSms *modem,
                                     GHashTable *properties,
                                     DBusGMethodInvocation *context);

static void impl_cdma_modem_sms_send (MMModemCdmaSms *modem,
                                     GHashTable *properties,
                                     DBusGMethodInvocation *context);

static void impl_cdma_modem_sms_send_from_storage (MMModemCdmaSms *modem,
                                                  guint idx,
                                                  DBusGMethodInvocation *context);

static void impl_cdma_modem_sms_set_indication (MMModemCdmaSms *modem,
                                               guint mode,
                                               guint mt,
                                               guint bm,
                                               guint ds,
...
#include "mm-modem-cdma-sms-glue.h"
...
static void
async_call_not_supported (MMModemCdmaSms *self,
                          MMModemFn callback,
                          gpointer user_data)
{
    MMCallbackInfo *info;
...
static void
sms_get_done (MMModemCdmaSms *self,
              GHashTable *properties,
              GError *error,
              gpointer user_data)
{
...
static void
sms_list_done (MMModemCdmaSms *self,
              GPtrArray *results,
              GError *error,
              gpointer user_data)
{
...
void
mm_modem_cdma_sms_send (MMModemCdmaSms *self,
                       const char *number,
                       const char *text,
                       const char *smsc,
                       guint validity,
                       ...
                       gpointer user_data)
{
    g_return_if_fail (MM_IS_MODEM_CDMA_SMS (self));
    g_return_if_fail (number != NULL);
    g_return_if_fail (text != NULL);
    g_return_if_fail (callback != NULL);

    if (MM_MODEM_CDMA_SMS_GET_INTERFACE (self)->send)
        MM_MODEM_CDMA_SMS_GET_INTERFACE (self)->send (self, number, text, smsc, validity, class, callback, user_data);
    else
...
sms_get_invoke (MMCallbackInfo *info)
{
    ...
    callback (MM_MODEM_CDMA_SMS (info->modem), NULL, info->error, info->user_data);
}

void
mm_modem_cdma_sms_get (MMModemCdmaSms *self,
                      guint idx,
                      ...
                      gpointer user_data)
{
    ...
    if (MM_MODEM_CDMA_SMS_GET_INTERFACE (self)->get)
        MM_MODEM_CDMA_SMS_GET_INTERFACE (self)->get (self, idx, callback, user_data);
    else {
        MMCallbackInfo *info;
...
void
mm_modem_cdma_sms_delete (MMModemCdmaSms *self,
                         guint idx,
                         MMModemFn callback,
                         gpointer user_data)
{
    ...
    if (MM_MODEM_CDMA_SMS_GET_INTERFACE (self)->delete)
        MM_MODEM_CDMA_SMS_GET_INTERFACE (self)->delete (self, idx, callback, user_data);
    else
        async_call_not_supported (self, callback, user_data);
}
...
void
mm_modem_cdma_sms_list (MMModemCdmaSms *self,
                       ...
                       gpointer user_data)
{
    ...
    if (MM_MODEM_CDMA_SMS_GET_INTERFACE (self)->list)
        MM_MODEM_CDMA_SMS_GET_INTERFACE (self)->list (self, callback, user_data);
    else {
        MMCallbackInfo *info;
...
void
mm_modem_cdma_sms_received (MMModemCdmaSms *self,
                           guint idx,
                           gboolean complete)
{
    g_return_if_fail (MM_IS_MODEM_CDMA_SMS (self));
...
void
mm_modem_cdma_sms_completed (MMModemCdmaSms *self,
                            guint idx,
                            gboolean complete)
{
    g_return_if_fail (MM_IS_MODEM_CDMA_SMS (self));
...
sms_delete_auth_cb (MMAuthRequest *req,
                    ...
                    gpointer user_data)
{
    MMModemCdmaSms *self = MM_MODEM_CDMA_SMS (owner);
    ...
    if (!mm_modem_auth_finish (MM_MODEM (self), req, &error)) {
        ...
        idx = info->num1;
        mm_modem_cdma_sms_delete (self, idx, async_call_done, context);
    }
}

static void
impl_cdma_modem_sms_delete (MMModemCdmaSms *modem,
                           guint idx,
                           DBusGMethodInvocation *context)
{
    GError *error = NULL;
                 ...
                 gpointer user_data)
{
    ...
    if (!mm_modem_auth_finish (MM_MODEM (self), req, &error)) {
        ...
        idx = info->num1;
        mm_modem_cdma_sms_get (self, idx, sms_get_done, context);
    }
}

static void
impl_cdma_modem_sms_get (MMModemCdmaSms *modem,
                        guint idx,
                        DBusGMethodInvocation *context)
{
    GError *error = NULL;
...
static void
impl_cdma_modem_sms_get_format (MMModemCdmaSms *modem,
                               DBusGMethodInvocation *context)
{
...
static void
impl_cdma_modem_sms_set_format (MMModemCdmaSms *modem,
                               guint format,
                               DBusGMethodInvocation *context)
{
    async_call_not_supported (modem, async_call_done, context);
...
static void
impl_cdma_modem_sms_get_smsc (MMModemCdmaSms *modem,
                             DBusGMethodInvocation *context)
{
    async_call_not_supported (modem, async_call_done, context);
}
...
static void
impl_cdma_modem_sms_set_smsc (MMModemCdmaSms *modem,
                             const char *smsc,
                             DBusGMethodInvocation *context)
{
    GError *error = NULL;
                  ...
                  gpointer user_data)
{
    ...
    if (!mm_modem_auth_finish (MM_MODEM (self), req, &error)) {
        ...
        mm_modem_cdma_sms_list (self, sms_list_done, context);
}

static void
impl_cdma_modem_sms_list (MMModemCdmaSms *modem,
                         DBusGMethodInvocation *context)
{
    GError *error = NULL;
...
static void
impl_cdma_modem_sms_save (MMModemCdmaSms *modem,
                         GHashTable *properties,
                         DBusGMethodInvocation *context)
{
    GError *error = NULL;
                  ...
                  gpointer user_data)
{
    ...
    if (error) {
        ...
        mm_modem_cdma_sms_send (self, number, text, smsc, validity, class, async_call_done, context);
}

static void
impl_cdma_modem_sms_send (MMModemCdmaSms *modem,
                         GHashTable *properties,
                         DBusGMethodInvocation *context)
{
    GError *error = NULL;
...
static void
impl_cdma_modem_sms_send_from_storage (MMModemCdmaSms *modem,
                                      guint idx,
                                      DBusGMethodInvocation *context)
{
    GError *error = NULL;
...
static void
impl_cdma_modem_sms_set_indication (MMModemCdmaSms *modem,
                                   guint mode,
                                   guint mt,
                                   guint bm,
                                   guint ds,
...
static void
mm_modem_cdma_sms_init (gpointer g_iface)
{
    ...
    signals[SMS_RECEIVED] =
        g_signal_new ("sms-received",
                      iface_type,
                      G_SIGNAL_RUN_FIRST,
                      G_STRUCT_OFFSET (MMModemCdmaSms, sms_received),
                      NULL, NULL,
    ...
    signals[COMPLETED] =
        g_signal_new ("completed",
                      iface_type,
                      G_SIGNAL_RUN_FIRST,
                      G_STRUCT_OFFSET (MMModemCdmaSms, completed),
                      NULL, NULL,
                      mm_marshal_VOID__UINT_BOOLEAN,
                      G_TYPE_NONE, 2, G_TYPE_UINT, G_TYPE_BOOLEAN);
...
GType
mm_modem_cdma_sms_get_type (void)
{
    static GType sms_type = 0;

    if (!G_UNLIKELY (sms_type)) {
        const GTypeInfo sms_info = {
            sizeof (MMModemCdmaSms), /* class_size */
            mm_modem_cdma_sms_init,   /* base_init */
            NULL,       /* base_finalize */
            NULL,
            NULL,       /* class_finalize */
            NULL,       /* class_data */
        ...
        dbus_g_object_type_install_info (sms_type, &dbus_glib_mm_modem_cdma_sms_object_info);
    }
